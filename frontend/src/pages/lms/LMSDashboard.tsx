...
import {
    ...
    FolderOpen, ArrowRight, Award, Users, BarChart2,
} from 'lucide-react';
...
export default function LMSDashboard() {
    ...
    const STATS = [
        { label: 'Enrolled', value: enrollments.length, icon: BookOpen, color: '#1B5E3B' },
        { label: 'In Progress', value: inProgressCount, icon: Clock, color: '#C8A046' },
        { label: 'Completed', value: completedCount, icon: CheckCircle2, color: '#A4372C' },
        { label: 'Courses', value: courses.length, icon: BarChart2, color: '#022512' },
    ];

    const QUICK_LINKS = [
        { icon: Calendar, label: 'Event Schedule', desc: 'Full tournament timeline', href: '/schedule', color: '#1B5E3B' },
        { icon: FolderOpen, label: 'Resources', desc: 'Study materials & downloads', href: '/resources', color: '#C8A046' },
        { icon: Award, label: 'Legacy Lab', desc: 'Innovation initiatives', href: '/legacy-lab', color: '#A4372C' },
        { icon: Users, label: 'Team', desc: 'Organizing committee', href: '/team', color: '#022512' },
    ];
            ...
            <main className="flex-1 overflow-y-auto md:min-h-screen">
                ...
                <header className="sticky top-0 z-20 bg-[#F6F0E1]/95 backdrop-blur-sm border-b border-[#022512]/10 px-6 md:px-8 py-4 flex items-center justify-between mt-[52px] md:mt-0">
                    <div>
                        ...
                        <p className="text-xs text-[#022512]/55 mt-0.5">PAUDC 2026 LMS Portal</p>
                    </div>
                ...
                <div className="px-6 md:px-8 py-7 space-y-9 max-w-6xl">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {STATS.map(({ label, value, icon: Icon, color }) => (
                            ...
                            >
                                ...
                                <div>
                                    <p className="text-xl font-black text-[#022512] leading-none">{value}</p>
                                    <p className="text-xs text-[#022512]/55 font-medium mt-0.5">{label}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <section>
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-base font-black text-[#022512]">My Courses</h2>
                            <Link
                                to="/lms/courses"
                        ...
                        {dataLoading ? (
                            ...
                            <div className="bg-white rounded-2xl p-10 text-center border border-[#022512]/5">
                                ...
                                <p className="font-bold text-sm text-[#022512]/60">No courses available yet</p>
                                <p className="text-xs text-[#022512]/40 mt-1">Course content will appear here</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                {courses.map(course => {
                    ...
                    {/* Quick Links */}
                    <section>
                        <h2 className="text-base font-black text-[#022512] mb-4">Quick Access</h2>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {QUICK_LINKS.map(({ icon: Icon, label, desc, href, color }) => (
                                ...
                                >
                                    <div className="rounded-xl p-2 shrink-0 mt-0.5" style={{ background: `${color}18` }}>
                                        <Icon className="w-4 h-4" style={{ color }} />
                                    </div>
                                    <div className="min-w-0">
                                        <p className="font-bold text-[#022512] text-xs leading-tight">{label}</p>
                                        <p className="text-xs text-[#022512]/45 mt-0.5 leading-tight">{desc}</p>
                                    </div>
                                </Link>
                            ))}
                        </div>
